...
namespace exec {
  ...
  u16 LDA(const Instruction& instr, Cpu& cpu) {
    switch (instr.addressing_mode) {
      ...
      case AddressingMode::IndexedZeroPageX: cpu.registers.a = ReadNextIndexedZeroPageX(cpu); cpu.Tick(); break;
  ...
  u16 LDY(const Instruction& instr, Cpu& cpu) {
    switch (instr.addressing_mode) {
      ...
      case AddressingMode::IndexedZeroPageX: cpu.registers.y = ReadNextIndexedZeroPageX(cpu); cpu.Tick(); break;
  ...
  u16 STA(const Instruction& instr, Cpu& cpu) {
    ...
    switch (instr.addressing_mode) {
      ...
      case AddressingMode::IndexedZeroPageX: WriteNextIndexedZeroPageX(cpu, cpu.registers.a); cpu.Tick(); break;
  ...
  u16 STY(const Instruction& instr, Cpu& cpu) {
    switch (instr.addressing_mode) {
      ...
      case AddressingMode::IndexedZeroPageX: WriteNextIndexedZeroPageX(cpu, cpu.registers.y); cpu.Tick(); break;
  ...
  u16 INC(const Instruction& instr, Cpu& cpu) {
    ...
    switch (instr.addressing_mode) {
      ...
      case AddressingMode::IndexedZeroPageX: addr = address::IndexedZeroPageX(cpu, cpu.ReadNext()); cpu.Tick(); break;
...
void Cpu::Step() {
  static const std::unordered_map<Op, std::function<u16(const Instruction&, Cpu&)>> instruction_map{
    { Op::NOP, exec::NOP },
    { Op::JMP, exec::JMP },
    { Op::JSR, exec::JSR },
    { Op::RTS, exec::RTS },
  ...
  const auto func_it = instruction_map.find(instr.op);
  if (func_it != instruction_map.end()) {
    new_pc = func_it->second(instr, *this);
  } else {
